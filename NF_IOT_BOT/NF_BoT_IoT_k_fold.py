...
method = "NF-BOT-IOT_train_FA_No_Smote"
...
# test_data = pd.read_csv('NF-BOT-IOT_test_WOA.csv')
...
num_unique_labels = np.unique(y_train)
...
file.close()
file = open(fname, 'a')
print("GradientBoostingClassifier comlepeted")
...
file.close()
file = open(fname, 'a')
print("XGBClassifier comlepeted")
...
file.close()
file = open(fname, 'a')
print("Gaussian_Naive_Bayes comlepeted")
...
file.close()
file = open(fname, 'a')
print("Completed reconstruction neural networks")
...
file.close()
file = open(fname, 'a')
print("Completed Bernoulli Naive Bayes with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed CatBoost with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Centralised Blending with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed BLoCNet with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed constructive_learning with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed AIS with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed GBBK with k-fold Cross-Validation")



"""## 51. Hidden Naïve Bayes (HNB) with k-fold Cross-Validation"""
...
result(all_y_pred_hnb, all_y_test_hnb, "Hidden Naïve Bayes (HNB)", time_to_train_hnb, time_to_predict_fold_hnb)

file.close()
file = open(fname, 'a')
print("Completed HNB with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed HistGradientBoostingClassifier with k-fold Cross-Validation")



"""## 53. IF (Isolation Forest) with k-fold Cross-Validation"""

# Initialize lists to store metrics
all_y_test_if = []
all_y_pred_if = []
start_cv_if = time.time()
time_to_predict_fold_if = 0
time_to_train_if = 0

# Perform k-fold cross-validation
for fold_number, (train_index_if, test_index_if) in enumerate(skf.split(X_train, y_train), 1):
    # Split data into train and test sets for the fold
    X_train_fold_if, X_test_fold_if = X_train.iloc[train_index_if], X_train.iloc[test_index_if]
    y_train_fold_if, y_test_fold_if = y_train.iloc[train_index_if], y_train.iloc[test_index_if]

    # Train the Isolation Forest model for this fold
    if_model_fold = IsolationForest(n_estimators=500,
                                    max_samples=0.5,
                                    contamination=0.1,
                                    max_features=1.0,
                                    bootstrap=True,
                                    random_state=7,
                                    n_jobs=-1)
    start_train_fold_if = time.time()
    if_model_fold.fit(X_train_fold_if, y_train_fold_if)
    end_train_fold_if = time.time()
    time_to_train_if += end_train_fold_if - start_train_fold_if

    # Predict using the trained model for this fold
    start_predict_fold_if = time.time()
    y_pred_fold_if = if_model_fold.predict(X_test_fold_if)
    end_predict_fold_if = time.time()
    time_to_predict_fold_if += end_predict_fold_if - start_predict_fold_if

    # Append metrics to lists
    all_y_test_if.extend(y_test_fold_if)
    all_y_pred_if.extend(y_pred_fold_if)

    # Generate confusion matrix and display for the fold
    cm_if = confusion_matrix(y_test_fold_if, y_pred_fold_if)
    plt.rcParams['figure.figsize'] = 8, 8
    sns.set_style("white")
    disp_if = ConfusionMatrixDisplay(confusion_matrix=cm_if)
    disp_if.plot(cmap=plt.cm.Blues)
    plt.title(f"Fold {fold_number} Confusion Matrix - IF (Isolation Forest)")
    pname_if = method + "_fold_" + str(fold_number) + "_IF_confusion_matrix.png"
    plt.savefig(pname_if)
    #plt.show()

# Calculate overall performance metrics
end_cv_if = time.time()
result(all_y_pred_if, all_y_test_if, "IF (Isolation Forest)", time_to_train_if, time_to_predict_fold_if)

file.close()
file = open(fname, 'a')
print("Completed IF (Isolation Forest) with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed ICA with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Lasso Regression with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Meta (KNN) with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Projected Gradient Descent (PGD) with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed PCA with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed SA with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Stacking Classifier with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Stacking Dilated Convolutional Autoencoders with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Temporal Deep Feedforward Neural Network with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed Voting Classifier with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed GBT with k-fold Cross-Validation")
...
file.close()
file = open(fname, 'a')
print("Completed CART with k-fold Cross-Validation")
